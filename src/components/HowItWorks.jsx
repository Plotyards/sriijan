...
const steps = [
  {
    ...
    description: 'Sign up and link your booked property (Project, Tower, Unit Number) on the secure Sriizan portal.',
    icon: Handshake,
    number: '01',
  },
  {
            ...
            {steps.map((step) => (
              <div key={step.title} className="relative group">
                <div className="liquid-glass-card rounded-3xl p-8 border border-white/80 flex flex-col items-center text-center relative z-10">
                  {/* Icon Circle */}
                  <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-600 to-emerald-700 text-white flex items-center justify-center mb-6 shadow-[0_10px_25px_rgba(5,150,105,0.35)] group-hover:scale-110 transition-transform duration-300 font-extrabold border border-emerald-400">
                    <step.icon className="w-9 h-9" />
                  </div>

                  {/* Step Number Badge */}
                  <div className="absolute top-6 right-6 text-4xl font-black text-slate-300 group-hover:text-emerald-600/30 transition-colors">
                    {step.number}
                  </div>
